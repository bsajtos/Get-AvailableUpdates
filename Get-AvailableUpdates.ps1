...
.DESCRIPTION
    ...
    The "Manageable" property shows the the result of the connection prerequisities in order (DNS -> PING -> WSMAN -> WMI -> 
    Invoke-Command) and stop processing further queries at the first error. This property represents an easy way to immediately
    assume what went wrong with a problematic server within remote query:
...
.PARAMETER PatchReport
    ...
    If this switch parameter is defined, a new gridview window will pop up with detailed available patchreport from each server.
...
.EXAMPLE
    ...
    Description
    -----------
    This command will search for updates on each of the target servers. Color coded summary sheet will be shown in the end. 
...
#>
    ...
    {
        Write-Error "PowerShell version 3+ is a must to run this function. Please try using the function from a machine with the`
	minimum supported PS version"
        break
    }
    ...
    $ComputerName = $ComputerName | Where-Object { $_ -ne "" } | ForEach-Object { $_.toupper().trimstart().trimend() } |` 
    ...
    $ScriptBlock = {
        ...
        $LastSync = Invoke-Command -ComputerName $compname -ScriptBlock {
            Start-Sleep -Seconds 3 #script too fast, log is not written fast enough. Read must be delayed a bit to get uptodate info
            $wsuslog = Get-Content -LiteralPath "C:\windows\SoftwareDistribution\ReportingEvents.log" -ea SilentlyContinue |`
